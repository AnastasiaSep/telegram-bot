...
def save_to_sheet(lang: str, data: dict, user_id: int, username: str):
     ...
     for header in headers:
         if header == cols["name"]:
             ...
             date_str = datetime.now(danang_tz).strftime("%d.%m.%Y %H:%M:%S")
             row.append(date_str)
...
def get_all_users() -> Dict[int, dict]:
    """Все user_id из русской и английской таблиц."""
    client = get_client()
    users: Dict[int, dict] = {}
    ...
    for lang in ("ru", "en"):
        try:
            ...
            tg_idx = headers.index("tg_username") if "tg_username" in headers else None

            for row in all_values[1:]:
                if len(row) <= user_id_idx or not row[user_id_idx].strip():
                    continue
                try:
                    uid = int(row[user_id_idx])
                except ValueError:
                    continue
                if uid in users:
                    continue
                tg = row[tg_idx].strip() if tg_idx is not None and len(row) > tg_idx else ""
                users[uid] = {"lang": lang, "tg_username": tg}
        except Exception as e:
            logging.error(f"get_all_users {lang}: {e}")

    return users


def save_event_response(lang: str, user_id: int, answer: str):
    """Записать ответ о посещении в колонку Присутствие / Presence."""
    client = get_client()
    sheet_id = SHEET_ID_RU if lang == "ru" else SHEET_ID_EN
    sheet = client.open_by_key(sheet_id).sheet1
    ...
    headers = [h.strip() for h in all_values[0]]
    presence_name = "Присутствие" if lang == "ru" else "Presence"

    if "user_id" not in headers:
        logging.error("Колонка user_id не найдена")
        return

    user_id_idx = headers.index("user_id")

    if presence_name not in headers:
        # добавить колонку в конец
        presence_idx = len(headers)
        sheet.update_cell(1, presence_idx + 1, presence_name)
        headers.append(presence_name)
    else:
        presence_idx = headers.index(presence_name)

    for i, row in enumerate(all_values[1:], start=2):
        if len(row) > user_id_idx and row[user_id_idx].strip() == str(user_id):
            sheet.update_cell(i, presence_idx + 1, answer)
            logging.info(f"Ответ user_id={user_id} записан: {answer}")
            return

    logging.warning(f"user_id={user_id} не найден в таблице {lang}")


async def ask_about_next_event(bot: Bot):
    """Рассылка вопроса о следующем мероприятии всем из таблиц."""
    users = get_all_users()
    logging.info(f"Рассылка вопроса {len(users)} юзерам...")

    ok, fail = 0, 0
    for user_id, user_info in users.items():
        lang = user_info["lang"]
        if lang == "ru":
            question = "Планируете ли вы посетить наше следующее мероприятие?"
            buttons = ["Да", "Нет"]
        else:
            question = "Will you attend our next event?"
            buttons = ["Yes", "No"]
        ...
        try:
            await bot.send_message(user_id, question, reply_markup=kb)
            ok += 1
            await asyncio.sleep(0.05)  # чуть паузы, чтобы не упереться в лимиты
        except Exception as e:
            fail += 1
            logging.error(f"Не отправилось {user_id}: {e}")

    logging.info(f"Рассылка готова: ok={ok}, fail={fail}")
    return ok, fail
...
# ===== Рассылка: следующее мероприятие =====
@router.message(Command("ask_next_event"))
async def cmd_ask_next_event(message: Message, bot: Bot):
    if message.from_user.id not in ADMIN_ID:
        return
    await message.answer("Отправляю вопрос всем участникам...")
    ok, fail = await ask_about_next_event(bot)
    await message.answer(f"Готово.\n✅ Отправлено: {ok}\n❌ Ошибок: {fail}")


@router.message(F.text.in_({"Да", "Нет", "Yes", "No"}))
async def handle_event_response(message: Message, state: FSMContext):
    # не перехватывать, если человек сейчас заполняет анкету
    ...
    lang = "ru" if answer in ("Да", "Нет") else "en"

    try:
        save_event_response(lang, user_id, answer)
    except Exception as e:
        logging.error(f"save_event_response: {e}")

    if lang == "ru":
        await message.answer(
            f"Спасибо! Ваш ответ: <b>{answer}</b>",
            reply_markup=ReplyKeyboardRemove(),
        )
    else:
        await message.answer(
            f"Thank you! Your answer: <b>{answer}</b>",
            reply_markup=ReplyKeyboardRemove(),
        )
